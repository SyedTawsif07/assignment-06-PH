            ...
            <div className="card w-96 bg-base-100 shadow-sm h-full">
                <div className="card-body">
                            ...
                            product.image === "resume" ? "/assets/products/portfolio.png" :
                                product.image === "automation" ? "/assets/products/operation.png" :
                                    product.image === "social" ? "/assets/products/social-media.png" :
                                        product.image === "stock" ? "/assets/products/design-tool.png" :
                                            "/assets/products/writing.png"
                        } alt="" />
                    </div>

                    <span className={`${product.tagType === "best-seller"
                        ? "bg-[#FEF3C6] text-[#BB4D00]"
                        : product.tagType === "popular"
                            ? "bg-[#E1E7FF] text-[#4F39F6]"
                            : "bg-[#DBFCE7] text-[#0A883E]"
                        } badge text-[14px] absolute top-4 right-4 p-3 rounded-3xl`}>
                        {product.tagType}
                    </span>
                    ...
                    <div className="mt-6">
                        <button
                            onClick={handleBtn}
                            className={`btn w-full rounded-3xl ${btn === 'buynow'
                                    ? 'bg-linear-to-r from-[#4F39F6] to-purple-600 text-white'
                                    : 'bg-green-600 text-white'
                                }`}
                        >
                            {btn === 'buynow' ? 'Buy Now' : 'Cart selected'}
                        </button>
                    </div>
